    ...
    if st.button("登录/注册"):
            ...
            if c == '登录':
                ...
                if user_name in users:
                    user_password = st.text_input("请输入密码：")
        ...
        def register():
            username = st.text_input("注册：请输入用户名")
            password = st.text_input("注册：请输入密码", type='password')
            ...
            if st.button("注册"):
                if username not in users:
                    ...
                    st.error("该用户名已被注册，请重新输入。")
        
        def login():
            global users
            entered_username = st.text_input("登录：请输入用户名")
            entered_password = st.text_input("登录：请输入密码", type='password')
        
            if st.button("登录"):
                if entered_username in users and users[entered_username] == entered_password:
                    st.success(f"欢迎，{entered_username}！登录成功！")
                else:
...
def page2():
    ...
    st.write("上传一张图片，你就会得到三张颜色失真的图片。快来试试吧！")
